...
```{r plot_horses_Canada, fig.width=10, fig.height=3, fig.cap = "Figure 1. Historical number of horses per province in Canada"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caption = "Horse population since 1940 in Canada")
```

Below we zoom in and look at the province of \`r params$province\`:

```{r plot_province, fig.width=5, fig.height=4, fig.cap="Figure 2. Horse populations for `r params$province` between 1940 - 1972."}
